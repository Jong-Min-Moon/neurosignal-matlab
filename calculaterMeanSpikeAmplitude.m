classdef calculaterMeanSpikeAmplitude < handle
    ...
    methods
        ...
        function [average_amp, aaa] = get_average_amp(cmsa, range_start, range_end, signal)
        % range_start = spike를 계산에 포함시키는 시간 범위 시작점
        % range_end = spike를 계산에 포함시키는 시간 범위 끝점
        start_number = cmsa.get_start_number(range_start);
        ...
        function start_number = get_start_number(cmsa, range_start)
            ...
            while ttt < cmsa.numberofspikes
                ...
                if ts_spike_peak_2 > range_start * cmsa.sf
