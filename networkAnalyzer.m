        ...
        function dist = spikeDist(na)
            ...
            for i = 1 : length(na.channelList)
                for j = i+1 : length(na.channelList)                    
                    ...
                    train_1 = py.numpy.array(train_1);
                    train_2 = py.numpy.array(train_2);
                    
                    train_1 = py.pyspike.SpikeTrain(train_1, py.list([0, nTimestamps]));
                    train_2 = py.pyspike.SpikeTrain(train_2, py.list([0, nTimestamps]));
                    
                    dist(i,j) = py.pyspike.spike_sync(train_1,train_2);
                    dist(j,i) = py.pyspike.spike_sync(train_1,train_2);
